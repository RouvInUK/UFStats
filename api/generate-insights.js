...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Aggregate and truncate data to ensure it fits comfortably within context
    ...
    // 3. Build a technical, dense match state payload
    ...
    // 4. Generate content from Gemini 3.5 Flash
    const response = await ai.models.generateContent({
      model: 'gemini-3.5-flash',
      contents: [
        {
          role: 'user',
          parts: [{ text: `Generate a structured, highly tactical coaching briefing. Here is the match state log: ${teamStateSummary}` }]
        }
      ],
      config: {
        systemInstruction: `You are 'Antigravity Coach Pro', an elite, highly analytical, and motivational Ultimate Frisbee coach. Deliver technical, encouraging huddle briefings and diagnostic suggestions. Focus on:
    ...
    // 5. Parse and return the JSON bundle
